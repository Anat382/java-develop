...
public class MainApp {
    ...
    public static int sumElementsArray(String[][] array) {
        ...
        for (int l1 = 0; l1 < array.length; l1++) {
            int sizeArr = array.length * array[l1].length;
            if (sizeArr != 16) {
                throw new AppArraySizeException("\nНекорректный размер массива: ", "count row: " + array.length + ", length row: " + array[l1].length);
            } else {
                for (int l2 = 0; l2 < array[l1].length; l2++) {
                    if (array[l1][l2] == null || !array[l1][l2].matches("\\d+")) {
                        throw new AppArrayDataException("Некорректный тип данных, не удалось преобразовать текст в int: ", "value: " + array[l1][l2] + ", index row: " + l1 + ", index value: " + l2 );
                    } else {
